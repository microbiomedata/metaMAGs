...
def phylodist_tsv(lineage_tsv, bins_dir, sdb, log):
 #based on imachen's phylodist script
 bininfo = dict()

 #per bin run phylodist computation
 for efile in os.listdir(bins_dir):
  lineage_h = {}
  epath = bins_dir + '/' + efile
  log.debug("running phylodist on file:" + epath)
  scaffs = []
  for record in SeqIO.parse(epath, "fasta"):
   scaffs.append(record.id)

  with open(lineage_tsv) as fh:
   for line in fh:
    linevals = line.rstrip().split('\t')
    escaff = linevals[0]
    if escaff not in scaffs : continue
    scaf_lineage = linevals[1]
    lineage_arr = scaf_lineage.split(";")
    lineage_str = ""
    for s3 in lineage_arr:
     if len(lineage_str) > 0: lineage_str += ";" + s3
