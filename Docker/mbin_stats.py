...
if __name__ == "__main__":
    ...
    sql_files = glob.glob(outdir + "/mbin*sdb")
    if len(sql_files) > 0:
    ...
    eukcc_file = glob.glob(outdir + "/eukcc_output/eukcc.csv.final")
    if len(eukcc_file) > 0:
        mag_list=parse_eukcc(eukcc_file[0],mag_list)
